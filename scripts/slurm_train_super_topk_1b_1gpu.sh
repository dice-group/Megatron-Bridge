#!/bin/bash
#
# Nemotron-3 Super 1B with topk routing — single GPU.
# Sister script to slurm_train_super_topany_1b_1gpu.sh; serves as the
# topk reference baseline (mirrors `small_topk` from sweep_routing_small.sh
# scaled to the 1B architecture).
#
# Usage:
#   sbatch scripts/slurm_train_super_topk_1b_1gpu.sh

#SBATCH --job-name=moe-topk-1b-1gpu
#SBATCH --nodes=1
...
#SBATCH --output=logs/topk_1b_%j.out
#SBATCH --error=logs/topk_1b_%j.err
...
CHECKPOINT_DIR="${CHECKPOINT_DIR:-/scratch/hpc-prf-merlin/luke/Megatron-Bridge/checkpoints_topk_1b_1gpu}"
RUN_NAME="${RUN_NAME:-super_topk_1b_1gpu}"

if [ -f "$PWD/.env" ]; then
    export $(grep -v '^#' "$PWD/.env" | xargs)
fi

# Topk baseline — no topany knobs apply. Mirror of small_topk.
ROUTING_TYPE="${ROUTING_TYPE:-topk}"
AUX_LOSS_COEFF="${AUX_LOSS_COEFF:-0}"
# These are inert for topk routing but kept for parity with the topany script:
...
echo "Routing   : $ROUTING_TYPE (aux=$AUX_LOSS_COEFF)"
...
echo "Model     : Nemotron-3 Super 1B (topk=6 reference baseline)"
echo "=============================="
